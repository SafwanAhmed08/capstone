...
# --------------------- MQTT SEND FUNCTION ---------------------
def send_to_hivemq(payload: dict):
    """Send a JSON payload to HiveMQ public broker."""
    ...
    print(f"✅ Published results to topic '{TOPIC}'")
    time.sleep(0.2)
    client.loop_stop()
    client.disconnect()
...
features = []
for window_start, group in windows:   # renamed to avoid shadowing time module
    if group.empty:
        continue
...
# prepare X_features and ensure columns match scaler
...
if hasattr(scaler, "feature_names_in_"):
    ...
    for col in expected:
        if col not in X_features.columns:
            X_features[col] = 0
    # remove any extra cols not in expected (to be safe)
    extra = [c for c in X_features.columns if c not in expected]
    if extra:
        X_features = X_features.drop(columns=extra)
    # reorder
...
# --------------------- Step 5: Publish Results as JSON ---------------------
payload = {
    "source": "IDS",
    "timestamp": dt.datetime.utcnow().isoformat() + "Z",
    "total_windows": len(results),
    "attack_count": int((results['prediction'] == 1).sum()),
    "alerts": [
        {
